...
!function(){"use strict";function e(e,o,n,t,i,r){function d(){var e=o.editor.project.get();if(t.name){var n=e.nodes.get(t.name);c.node=n.copy(),c.original=n,c.action="Update"}else c.node=new b3e.Node,c.node.category="composite";var i=[];e.nodes.each(function(e){e.name!==c.node.name&&i.push(e.name)}),c.denylist=i.join(",")}function a(){var e=o.editor.project.get();c.original?e.nodes.update(c.original,c.node):e.nodes.add(c.node),n.go("editor"),r.success("Node created","Node has been created successfully.")}function s(){i.confirm("Remove node?","Are you sure you want to remove this node?\n\nNote: all blocks using this node will be removed.").then(function(){var e=o.editor.project.get();e.nodes.remove(c.original),r.success("Node removed","The node has been removed from this project."),n.go("editor")})}var c=this;c.action="New",c.node=null,c.denylist=null,c.original=null,c.save=a,c.remove=s,d()}angular.module("app").controller("EditNodeController",e),e.$inject=["$scope","$window","$state","$stateParams","dialogService","notificationService"]}();
